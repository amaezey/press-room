...
UPLOAD_JS = r"""
// ---------------------------------------------------------------- source
// The press works on two arrays: a density plate and a silhouette. The
// built-in default
...
function rebuild(){
  if (!SOURCE){ defaultPlates(); return; }
  ...
  fillHoles(mask);
  dropSpecks(mask);
  // the cleaned shape gates the ink, or the specks we just binned would still
  // print as dirt. It gates a carved shape too: that is what Cutout is for, so
  // the slider runs from the full frame at 0 to a tight shape at the top.
  for (let i = 0; i < n; i++) blue[i] *= mask[i];
  const body = new Float32Array(n);
  for (let i = 0; i < n; i++) body[i] = mask[i];

  el('rowCut').classList.toggle('off', cutout);
  setPlates(blue, body, (cutout ? 'Transparency' : 'Cutout') + ' · ' + SOURCE.w + '×' + SOURCE.h);
}
...
// The default image arrives already traced, so Ink from cannot re-read a picture. It
// can still turn the density over inside the silhouette, which is the same
// thing the switch does to an upload.
function defaultPlates(){
  if (!DEFAULT_PLATES.blue) return;
  const src = DEFAULT_PLATES.blue, body = DEFAULT_PLATES.body;
  let blue = src;
  if (val('flip') === 1){
    blue = new Float32Array(src.length);
    for (let i = 0; i < src.length; i++) blue[i] = (1 - src[i]) * body[i];
  }
...
for (const k of ['blue','body']){
  ...
  im.onload = () => {
    ...
    if (DEFAULT_PLATES.blue && DEFAULT_PLATES.body){
      const claimed = picked;   // did anything land while the default was loading
      useDefault();
      picked = claimed;
      idb(db => {
...
function takeFile(f){
  if (!f) return;
  picked = true;
  if (f.size > 40e6){ note('Over 40 MB'); return; }
  keep(f);
  useBlob(f, f.name);
}

el('file').onchange = e => takeFile(e.target.files && e.target.files[0]);

// drop a picture anywhere on the page. The browser opens a dropped file in the
// tab unless every dragover is cancelled, so both handlers sit on the window.
...
function randomise(){
  ...
  for (const id of ['shs1','hls1','shs2','hls2']) el(id).value = between(35, 100);
  ...
  el('order').checked = chance(0.3); el('front').checked = !el('order').checked;
  ...
  // no ceiling on these: any number of them, anywhere on the travel
  ['reg','den','gra','dry'].forEach(id => {
...
@media(max-width:820px){{
  html,body{{height:auto}} body{{overflow:visible}}
  .wrap{{flex-direction:column-reverse}} .stage,.panel{{flex:none}}
  .stage{{position:sticky;top:0;z-index:1;background:var(--sheet);
  border-bottom:1px solid var(--edge)}}
  .panel{{width:100%;overflow:visible;border-right:0}}
...
body.dropping .stage{{outline:2px dashed var(--edge);outline-offset:-10px}}
body.dropping .file{{border-color:var(--edge);background:#F4F3EE}}
  ...
  <div class="panel">
    ...
    <fieldset><legend>Background</legend>
      <div class="row"><label>Background</label>
        <select id="sub">{opts(SUBSTRATES[:-1])}</select><output></output></div>
    </fieldset>
    ...
    <fieldset><legend>Front face</legend>
      ...
      <div class="row" id="rowShs1"><label>Shadow mix</label>
        <input type="range" id="shs1" min="0" max="100" value="60"><output>60</output></div>
      ...
      <div class="row" id="rowHls1"><label>Highlight mix</label>
        <input type="range" id="hls1" min="0" max="100" value="60"><output>60</output></div>
    </fieldset>
    ...
    <fieldset><legend>Back face</legend>
      ...
      <div class="row" id="rowShs2"><label>Shadow mix</label>
        <input type="range" id="shs2" min="0" max="100" value="60"><output>60</output></div>
      <div class="row"><label>Highlights</label>
        <select id="hil2"><option value="-1">Same as ink</option>{opts(INKS, -1)}</select><output></output></div>
      <div class="row" id="rowHls2"><label>Highlight mix</label>
        <input type="range" id="hls2" min="0" max="100" value="60"><output>60</output></div>
    </fieldset>
    ...
    <fieldset><legend>Reproduction</legend>
      ...
      <div class="row" id="rowOrder"><label>On top</label>
        <div class="seg"><button type="button" data-id="front">Front face</button>
        <button type="button" data-id="order">Back face</button></div>
        <input type="radio" name="ord" id="front" checked hidden>
        <input type="radio" name="ord" id="order" hidden><output></output></div>
    </fieldset>
...
// the band inks are mixed once, not per pixel: at 0 the band keeps the main
// ink, at 100 it is the band ink outright, which is what it used to do always
function colourFn(inkHex, shaHex, hilHex, paperRGB, shaAmt, hilAmt){{
  const ink = hex2rgb(inkHex);
  const sha = shaHex? mix(ink, hex2rgb(shaHex), shaAmt) : null;
  const hil = hilHex? mix(ink, hex2rgb(hilHex), hilAmt) : null;
  return (v) => {{
...
function sync(){{
  ...
  el('rowOrder').classList.toggle('off', !two);
  const p = PROCS[val('proc')];
  const live = p.screen !== 'none';
  el('cell').closest('.row').classList.toggle('off', !live);
  el('cell').disabled = !live;
  for (const f of ['1','2']){{
    for (const [sel, row, sld] of [['sha'+f,'rowShs'+f,'shs'+f],
                                   ['hil'+f,'rowHls'+f,'hls'+f]]){{
      const live = val(sel) >= 0 && (f === '1' || two);
      el(row).classList.toggle('off', !live);
      el(sld).disabled = !live;
    }}
  }}
...
function bind(){{
  ...
  el('reset').onclick = () => {{
    ...
    el('shs1').value=60; el('hls1').value=60; el('shs2').value=60; el('hls2').value=60;
    ...
    el('ko').checked=true; el('over').checked=false; el('order').checked=false; el('front').checked=true;
